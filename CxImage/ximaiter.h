...
public:
	...
	void Reset ();
	void Upset ();
	void SetRow(const uint8_t *buf, size_t n);
	...
	void GetSteps(int32_t *x, int32_t *y) const {  *x = Stepx; *y = Stepy; }
	BOOL NextStep();
	BOOL PrevStep();

	void SetY(int32_t y);	/* AD - for interlace */
	int32_t  GetY() const {return Ity;}
	BOOL GetCol(uint8_t* pCol, uint32_t x);
	BOOL SetCol(const uint8_t* pCol, uint32_t x);
};
...
{
	IterImage = ima ? ima->GetBits() : NULL;
	Itx = Ity = 0;
	Stepx = Stepy = 0;
}
/////////////////////////////////////////////////////////////////////
inline
CImageIterator::operator CxImage* () const
{
	return ima;
}
/////////////////////////////////////////////////////////////////////
inline BOOL CImageIterator::ItOK() const
{
...
/////////////////////////////////////////////////////////////////////
inline void CImageIterator::SetRow(const uint8_t *buf, size_t n)
{
//	if (n < 0)
//		n = ima->GetEffWidth();
//	else
...
{
	if ((pCol == nullptr) || (ima->GetBpp() < 8) || (x >= ima->GetWidth()))
		return 0;
...
/////////////////////////////////////////////////////////////////////
inline BOOL CImageIterator::SetCol(const uint8_t* pCol, uint32_t x)
{
	if ((pCol==nullptr)||(ima->GetBpp()<8)||(x>=ima->GetWidth()))
		return 0;
	uint32_t h = ima->GetHeight();
